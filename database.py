...
class Database(object):
    ...
    def __init__(self, my_dojo):
        self.db = None
        self.my_dojo = my_dojo
    ...
    def save_state(self, args):
        ...
        if args["--db"]:
            ...
            self.db = create_engine("sqlite:///dojo.db")
        self.db.echo = False
    ...
    def add_people(self):
        ...
        for person in self.my_dojo.people:
            if person not in self.already_added:
                if person.job_type == "Fellow":
                    is_fellow = True
                else:
                    is_fellow = False
                if person in self.my_dojo.allocated_people:
                    is_allocated = True
                else:
                    is_allocated = False
    ...
    def add_rooms(self):
        ...
        if rooms.exists():
            ...
            rooms.create()
        insert = rooms.insert()
        for room in self.my_dojo.rooms:
            if room not in self.already_added:
                if room.room_type == "Office":
                    is_office = True
                else:
                    is_office = False
                if room in self.my_dojo.vacant_rooms:
                    is_vacant = True
                else:
                    is_vacant = False
                insert.execute(
    ...
    def load_state(self, args):  # pragma: no cover
        ...
        for row in people:
            ...
            person_from_db = self.my_dojo.people[-1]
        ...
        for table_name in table_names:
            ...
            for row in room:
                ...
                self.my_dojo.reallocate_person({
                    "<employee_id>": emp_id,
